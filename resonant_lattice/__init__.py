...
__version__ = "1.6.9"  # grok narrative spans the whole session. summarize_session saw only the last 40 episodes, so granite's --context-shift clipped the oldest and a long multi-window grok ingest narrated only its tail (a thin "paused for compact" gist). rlm_ingest.py now builds a hierarchical, born-fact-grounded digest of the WHOLE session (per-window born facts + open/decision roll-up + head/mid/tail user-turn spine) and hands it to summarize_session via a new optional digest= param that skips the 40-episode cap; a tuned "remembered arc" narrative_prompt (throughline / locked decisions / open loops with resumable handles / closed work in past tense), _ascii_sanitize on every stored narrative (em/en dashes, smart quotes, ellipsis -> ASCII), and the length cap raised 1500 -> 2200 for the richer structured form. No extra LLM calls; backward-compatible (no digest -> original episode path, hermes narrative unchanged). Verified on a real 12-window session: a 373-char tail-only gist became a 1368-char full-span arc. Prior 1.6.8 = grok re-compact dedup (ingest_watermarks high-water mark; append-only chat_history, a second /compact mines only the new tail).
