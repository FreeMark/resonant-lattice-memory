...
__version__ = "1.7.1"  # residual polish (grok aperture), doc-only for the core. GROK PREFETCH CLOCK: the per-turn prefetch MCP handler now stamps a model-friendly <current_datetime> at SERVE time (mirroring the hermes recall._time_context_block that grok's side-channel lacked), using client-local time, so a cached block still carries a fresh wall clock to judge open-loop staleness. NARRATIVE PROJECTION: the ## narrative header now shows the live memory_cycle (so the per-row 'now (cycle N)' reads as as-of, not the current clock) and the newest arc surfaces up to 2 'closed:' loops (a resumed-then-finished item stops reading as still-open). DOCS: dual narrative-prompt path clarified (freeform narrative_prompt vs opt-in narrative_structured JSON) in prompts.py + config_schema.py + both READMEs; grok config entity_vocabulary + the grok README currency (rlm_narrative/rlm_dream in prose, structured/historical narrative, watermark dedup). No dial changes; hermes runtime behavior unchanged (comment + description strings only). Prior 1.7.0 = dream/narrative enhancement. NARRATIVE: rebuilt from a hierarchical, born-fact-grounded HYBRID digest of the whole session (per-window born facts + open/decision roll-up + a head/mid/tail spine of the ACTUAL dialogue, both roles), replacing the tail-40-episodes input that clipped long ingests; a blind cloud-judged A/B/C proved the hybrid beats facts-only on coverage with no added fabrication. Narratives are now STRUCTURED (typed throughline/decisions/open_loops/closed/topics; summarize_session(structured=True) -> JSON with freeform fallback), temporally FRAMED (historical flag + mark_prior_narratives_historical as a full two-direction recompute, so a row that becomes newest is cleared, not left stale), ASCII-GUARANTEED (_ascii_sanitize folds accents + drops residual non-ASCII), surfaced by a cycle-ordered projection (newest arc full, older one-liners) and a new read-only rlm_narrative MCP tool. OBSERVABILITY: new read-only rlm_dream MCP tool + node script (a consolidation report card - tier flow + promotion-ready counts, dwell, decay, contested, abstraction/gist, dials in effect). HYGIENE: extraction prompt rejects the lattice's own transient state readings (cycle/tier/count gauges); 20 fossils pruned. MCP surface 16 -> 18 tools. Backward-compatible (structured/digest opt-in; hermes episode path unchanged); node test suite 140 -> 146. Prior 1.6.9 = P0 whole-session digest + repo-wide dash sweep.
