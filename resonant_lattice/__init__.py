...
__version__ = "1.7.4"  # RELATION EXTRACTION AND RELATIONAL QUERY, BOTH UNBLOCKED. Three silent failures, each found by measuring a live 3,412-fact FHE corpus rather than by a raised error. (1) _llm_extract_triples spliced text[find("[") : rfind("]")+1] into json.loads; a reasoning model emits a COMPLETE valid array and then keeps talking -- trailing prose, or a whole SECOND array after it reconsiders -- so the span covered array1 + prose + array2, json.loads raised Extra data, and the blanket except returned [] indistinguishably from "this fact states no relations". 8 of 23 measured calls lost every triple that way. first_json_array() takes the first well-formed array via raw_decode, scanning successive "[" candidates because prose can contain one, and keeps None (parse failure) distinct from [] (nothing here). Re-extracting one corpus: 2,866 -> 5,723 triples, zero-relation facts 71.8% -> 42.1%, node recurrence 36.1% -> 39.6%, largest component 82.5% -> 88.0% -- the graph densified rather than dissolving. (2) _QUERY_REL_KEYWORDS is a generic personal-assistant set (has, uses, lives_in, works_at); a corpus with a closed relation_vocabulary shares none of it, yet a detected match still filtered the walk, so "What HAS changed in bootstrapping research?" returned nothing while bootstrapping was a hub with 473 edges. An out-of-vocabulary detection is now ignored (anchor-only traversal); an in-vocabulary one still filters. The store also never received relation_vocabulary -- the query path could not know its own predicates. (3) relational_recall ordered a hub anchor by confidence alone, so the answer was independent of the question: two different questions on a 141-edge anchor returned the SAME triples while the ones that answered them sat below max_results. Candidates are now ranked by overlap with the query text before truncation; a structured call and a query matching nothing are unaffected. Node suite 198 -> 204.
