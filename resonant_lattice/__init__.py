...
__version__ = "1.7.5"  # TRIMMING AN ARGUMENT MUST NOT UNBALANCE ITS BRACKETS. _resolve_arg trimmed relation arguments with s.strip("\"'.,;:!?()[]{}"). str.strip(chars) has no notion of balance, so an argument that legitimately ENDS in a closing bracket lost it: B_Ks2_YASHE(t) -> B_Ks2_YASHE(t, o(ln log n) -> o(ln log n, 3 * 2^{64} -> 3 * 2^{64. Measured on a live 74,813-fact corpus, 630 of 10,572 stored arguments (5.96%) were corrupted this way, and the extractor had been correct every time -- the damage was entirely in the normalizer. Trimming now only removes a bracket when doing so leaves the argument balanced. THE REPAIR RECOVERED ZERO STRUCTURE, AND THAT IS THE INTERESTING PART: the prediction was that truncation had split one object into two nodes, but the corruption was UNIFORM -- every instance of a given argument truncated identically -- so 0 collisions existed to merge. The fix therefore buys correctness (the identifier matches the paper it came from) and prevents FUTURE fragmentation between old corrupted rows and every correct row written from now on. 1,055 arguments repaired; 51 left alone, being unmatched CLOSERS with no single correct completion. Suite 204 -> 210.
