...
__version__ = "1.7.2"  # OBSERVABILITY + EXTRACTION INTEGRITY. New extraction_audit table: one row per consolidation epoch (attempts, quoteless_retries, facts, quoted, transcript_chars, transcript_quotes, synthesis, outcome), written before the write lock and never raising, because hermes emits NO stderr and every logger call in this plugin has gone to a void on every run -- a deployed retry guard could not be evaluated at all. QUOTELESS RETRY THAT ACTUALLY DIFFERS: a wholesale source_quote omission now retries with a correction naming the omission and escalating temperature (0.1/0.45/0.7); the previous version re-sent an identical payload and a live run measured 5 attempts returning the same 44 quoteless facts. quoteless_max_retries (default 2) caps it separately, and an identical corrective result stops at once as quoteless_deterministic. Measured over one 131-block campaign: 11 firings in 197 sessions, 8 recovered, no_quote facts 11.6% -> 1.4% of corpus. ABSTRACTION UNBLOCKED: _find_semantic_match gains exclude_ids (k inflated to 1+len, since non-vector predicates filter AFTER the k nearest) so a synthesis is no longer deduped against its own inputs -- measured 9 of 9 candidates rejected, 8 against their own sources, leaving a 6,983-fact lattice with ONE abstraction after 195 consolidations. perform_abstraction_pass and consolidate_before_prune take embed_endpoint, because reusing the reason endpoint for embeddings silently inserted abstractions with no vector when an OpenAI-compatible shim fronted the reasoning model. Also: mechanical source_ref recovery from tool results (default OFF), think:false on every reason call, dream cadence governs the session-end dream, FinalizeLock held for the WRITE phase only. Node suite 164 -> 186. Prior 1.7.1 = residual polish (grok aperture), doc-only for the core. GROK PREFETCH CLOCK: the per-turn prefetch MCP handler now stamps a model-friendly <current_datetime> at
