...
__version__ = "1.7.3"  # PORTABLE RELATIONS + RECALL QUALITY. The closed relation vocabulary now works outside the operational domain it was designed in -- five config gaps, all silent, found by tracing a live 6,983-fact clinical corpus: relation_subject_kinds (the built prompt hardcoded "components, machines, services...", so a clinical lattice returned [] while enforcing the vocabulary perfectly), relation_attribute_predicates (attribute objects for predicates like reference_interval survive relation_require_entity), value-preserving _resolve_arg(is_value=) (an attribute object is no longer collapsed to its matched entity, so stage_defined_by objects keep their numbers), alias substitution scoped to the matched span (an alias no longer replaces the WHOLE argument), relation_num_predict (uncapped relation calls ran to the endpoint default and timed out into a silent []), plus positional [[s,r,o]] triple parsing. Relational queries are abbreviation-safe: _QUERY_NON_ANCHORS drops pronouns/demonstratives as a FINAL pass over anchors from every path, with an ALL-CAPS ESCAPE so a corpus storing HE (hepatic encephalopathy) keeps it queryable while a mid-question "He" no longer binds to it. Attestation gains an opt-in content<->quote NUMERIC-CONFLICT check for the blind spot where a fact contradicts its own quote: requires a NEAR MISS and exempts rounding (measured on 6,640 quoted facts: naive rule 84 flags, near-miss 14, rounding-exempt 10, the real contradiction caught at every tolerance), MARKS numeric_conflict and never drops, default off. Recall gains recall_redundancy_ceiling (default 0.0 = off), a read-time redundancy gate: a top-k candidate too cosine-close to an already-chosen row is APPENDED after diverse ones, never dropped -- k preserved, the rank-1 answer survives every measured ceiling -- because a corpus census showed 16,432 pairs at or above the store's own 0.78 similarity_threshold with 65% carrying DIFFERENT numbers, so 0.78 is a topic threshold and store-side dedup would delete real content. Node suite 186 -> 198.
